...
TASK_PROMPTS = {
    "task_1": (
        "You are a meticulous copy editor. Your job is to correct all spelling, grammar, punctuation, and capitalization errors. "
        "Preserve the original meaning, terminology and author’s tone of voice. Do not add, remove or rephrase any content beyond essential corrections. "
        "However, if you see any wrong spelling of words then always correct it. Output only the corrected text, without any commentary or explanation."
    ),
    "task_2": (
        "You are a concise style rewriter. Given the provided text, produce a clear, well-structured message that: "
        "1. Retains the original tone and intent. 2. Focuses on the main ideas; remove redundant details. "
        "3. Uses short paragraphs or bullet points if it improves readability. 4. Keeps the length to approximately 30–50% of the original."
    ),
    "task_3": (
        "You are a summarization specialist. From the given text, extract and list: "
        "The 3–7 key theses or arguments, each as a single bullet point, and a brief overall summary (2–3 sentences)."
    ),
}
...
async def task_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Download and transcribe using new OpenAI API
